...
food = function(t){
  ...
  D = 6
  tt = t %% 24
  food = 0.5*(((tt>=7)&(tt<7.5))*B + ((tt>=12)&(tt<12.5))*L + ((tt>=18)&(tt<19))*D)
...
plot(sol[,1],sol[,2],type = 'l', lwd = 6, col = 'blue',
     ...
     main = 'Tryptophan in the Gut and Brain (Keto)')
...
plot(sol[,1],sol[,4],type = 'l', lwd = 6, col = 'blue',
     ...
     main = '5-HTP in the Gut and Brain (Keto)')
...
plot(sol[,1],sol[,6],type = 'l', lwd = 6, col = 'blue',
     ...
     main = 'Serotonin in the Gut and Brain (Keto)')
